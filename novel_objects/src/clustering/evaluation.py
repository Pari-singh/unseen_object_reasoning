...
from novel_objects.src.clustering.utils import seed_torch, torch_distance, \
    posemb_sincos_2d, visualization_eval
...
class Evaluation:
    def __init__(self, args):
        ...
        self.kmeans = pickle.load(open(os.path.join('kmeans', args.saved_kmeans), 'rb'))
        self.cluster_centers = self.kmeans.cluster_centers_

    def eval(self, inference_loader):
        cluster_composition = np.load(os.path.join('cluster_compositions', self.args.cluster_composition), allow_pickle=True)
        raw_imgs = []
        targets = np.array([])
        ...
        vis_superclass = [[] for _ in range(len(ghc.superclasses_map))]
        for i, instance in enumerate(all_preds_distribution):
            instance_class = np.array([0.0 for _ in range(100)])
            ground_truth_superclass = ghc.superclasses_map[ghc.class_to_superclass[targets[i]]]
            ...
            superclass_list = np.array([0.0 for _ in range(len(ghc.superclasses_map))])
            for k, score in enumerate(instance_class):
                superclass_list[ghc.superclasses_map[ghc.class_to_superclass[k]]] += score
            superclass_list_arg = np.argsort(superclass_list)[::-1]
            vis_superclass[superclass_list_arg[0]].append(i)
            # superclass_list_arg = [4, 1, 5, 6, 2, 0, 3]
        ...
        visualization_eval(self.args, vis_superclass, raw_imgs)
