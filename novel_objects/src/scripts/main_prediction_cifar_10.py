...
eval_only = False
if use_augmentation and not eval_only:
...
test_set = CIFAR10(root=DATASET_PATH, train=False, download=True)
...
class Decoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.net = nn.Sequential(
            ...
            act_fn(),
            nn.Dropout2d(0.7),
            nn.ConvTranspose2d(c_hid, c_hid, kernel_size=3, output_padding=1, padding=1,
                               stride=2),  # 16x16 => 32x32
            act_fn(),
            nn.Conv2d(c_hid, num_classes, kernel_size=3, padding=1),
            nn.Dropout(0.3),
            nn.Softmax()
...
class Autoencoder(pl.LightningModule):
    ...
    def __init__(self,
        ...
        if mode_loss_weights is None:
            self.mode_loss_weights = [1.0, 0.0]
...
exp_id = 2
latent_dim = 384
ckpt_file_name = 'epoch=612-step=215162.ckpt'
...
class_names_cifar_10_rearranged = ['airplane', 'automobile', 'ship', 'truck',
                                   'bird', 'cat', 'deer',
                                   'dog', 'frog', 'horse']
ckpt_path = 'src/saved_models/cifar10_' + str(latent_dim) + '/lightning_logs/version_' + str(exp_id) + '/checkpoints'
...
for i in range(len(val_loader.dataset.indices)):
    ...
    if True:
        ...
        heatmap_img = cv2.applyColorMap(np.uint8(x_hat[predicted_class, :, :] * 255), cv2.COLORMAP_JET)
        super_imposed_img = cv2.addWeighted(heatmap_img, 0.3, batch[3], 0.7, 0)
        plt.imshow(super_imposed_img)
        plt.imsave(os.path.join(os.getcwd(), 'output', str(i)+'_imp.png'), super_imposed_img)
        plt.subplot(1,2,1), plt.imshow(heatmap_img)
        plt.title('Pred: ', predicted_class)
        plt.subplot(1,2,2), plt.imshow(batch[3])
        plt.title('GT: ',class_names_cifar_10[sample_class])
        plt.savefig(os.path.join(os.getcwd(), 'output', str(i)+'_hm_img.png'))
    if np.mean(x_hat[predicted_class,:,:]) < 0.65:
        plt.imshow(x_hat[predicted_class, :, :])
        import cv2
        heatmap_img = cv2.applyColorMap(np.uint8(x_hat[predicted_class,:,:]*255), cv2.COLORMAP_JET)
        super_imposed_img = cv2.addWeighted(heatmap_img, 0.2, batch[3], 0.8, 0)
        plt.imshow(super_imposed_img)
