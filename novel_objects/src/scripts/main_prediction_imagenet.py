...
# Torchvision
import torchvision
from novel_objects.src.dataset.imagenet import ImageNetMini as ImageNetMini
...
random.seed(42)
split_proportion = {'train': .85, 'val': .15}
train_split = int(len(train_dataset.targets)*split_proportion['train'])
val_split = int(len(train_dataset.targets)) - train_split
train_set, val_set = torch.utils.data.random_split(train_dataset, [train_split, val_split])
...
total_classes_train = len(train_set.dataset.classes)
num_train_classes = np.uint32(np.ceil(total_classes_train*train_test_classes[0]))  # TODO: DO NOT HARDCODE
num_test_classes = len(train_set.dataset.classes) - num_train_classes
train_classes, test_classes = split_train_test(lengths=[num_train_classes, num_test_classes], total_classes=train_dataset.combined_classes)
...
train_split, val_split = int(len(train_idx)*0.8), len(train_idx)-int(len(train_idx)*0.8)  # TODO: DO NOT HARDCODE
...
# Loading the test set
# test_set = ImageNetMini(root=DATASET_PATH, train=False, transform=transform, download=True)
...
class Autoencoder(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.decoder = decoder_class(num_input_channels, base_channel_size, latent_dim, num_classes=num_classes)
...
def train_imagenet(latent_dim, use_pretrained=False, use_mode_loss=True, mode_loss_weights=None):
    # Create a PyTorch Lightning trainer with the generation callback
    trainer = pl.Trainer(default_root_dir=os.path.join(CHECKPOINT_PATH, f"imagenet_mini_{latent_dim}"),
    ...
    pretrained_filename = os.path.join(CHECKPOINT_PATH, f"ImageNetMini_{latent_dim}.ckpt")
    if os.path.isfile(pretrained_filename) and use_pretrained:
        ...
        model = Autoencoder(base_channel_size=32, latent_dim=latent_dim, use_mode_loss=use_mode_loss,
                            mode_loss_weights=mode_loss_weights, width=width, height=height, num_classes=num_classes)
...
if not eval_only:
    for latent_dim in [512]:
        model_ld, result_ld = train_imagenet(latent_dim, use_pretrained=False,
                                             use_mode_loss=use_mode_loss, mode_loss_weights=mode_loss_weights)
        model_dict[latent_dim] = {"model": model_ld, "result": result_ld}
        torch.save(model_ld.state_dict(), os.path.join(CHECKPOINT_PATH, 'ImageNet_'+str(latent_dim)))
...
exp_id = 28
latent_dim = 512
ckpt_file_name = 'epoch=451-step=105767.ckpt'
class_names_eval = [trainval_dataset.classes[x] for x in train_classes]

ckpt_path = 'saved_models/imagenet_mini_' + str(latent_dim) + '/lightning_logs/version_' + str(exp_id) + '/checkpoints'
