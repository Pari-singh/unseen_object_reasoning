...
class CustomFeatureCIFAR100(VisionDataset):
    """`CIFAR100 <https://www.cs.toronto.edu/~kriz/cifar.html>`_ Dataset.
    ...
    def __init__(
        ...
        super(CustomFeatureCIFAR100, self).__init__(root, transform=transform,
        ...
        if feature_model:
            self.feature_model.to(torch.device('cuda:0'))
        ...
        #                 if counter[self.classes[entry['labels'][i]]] < 600:
        #                     self.targets.append(self.class_to_idx[self.classes[entry['labels'][i]]])
        #                     self.data.append(entry['data'][i])
        #                     counter[self.classes[entry['labels'][i]]] += 1
        #
        ...
        #                 if counter[self.classes[entry['fine_labels'][i]]] < 600:
        #                     self.targets.append(self.class_to_idx[self.classes[entry['fine_labels'][i]]])
        #                     self.data.append(entry['data'][i])
        #                     counter[self.classes[entry['fine_labels'][i]]] += 1
    ...
    def _load_meta(self) -> None:
        ...
        with open(path, 'rb') as infile:
            data = pickle.load(infile, encoding='latin1')
            self.classes += data[self.meta['key']]
            # self.classes_acc.append(data[self.meta['key']])
        self.class_to_idx = {_class: i for i, _class in enumerate(self.classes)}
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        pdb.set_trace()
        img.to(torch.device('cuda:0'))
        feature_vector, _ = self.feature_model.get_last_selfattention(img.unsqueeze(0))

        return feature_vector, target, index, raw_img
